...
package com.BrotherHoodOfDiethylamide.OrangeSunshine.ivtoolkit.legacy.blocks;

import com.BrotherHoodOfDiethylamide.OrangeSunshine.ivtoolkit.blocks.BlockPositions;
import com.BrotherHoodOfDiethylamide.OrangeSunshine.ivtoolkit.blocks.IvTileEntityHelper;
import com.BrotherHoodOfDiethylamide.OrangeSunshine.ivtoolkit.legacy.models.IvMultiBlockHelper;
import com.BrotherHoodOfDiethylamide.OrangeSunshine.ivtoolkit.legacy.raytracing.IvRaytraceableAxisAlignedBox;
...
import com.BrotherHoodOfDiethylamide.OrangeSunshine.ivtoolkit.tools.EnumFacingHelper;
...
public class IvTileEntityMultiBlock extends TileEntity
{
    ...
    public List<Double> centerCoords = Arrays.asList(new Double[]{0.5, 0.5, 0.5});
    public List<Double> centerCoordsSize = Arrays.asList(new Double[]{0.5, 0.5, 0.5});
    ...
    public double[] getActiveCenterCoords()
    {
        ...
        return new double[]{getPos().getX() + centerCoords.get(0), getPos().getY() + centerCoords.get(1), getPos().getZ() + centerCoords.get(2)};
    }
    ...
    public void readFromNBT(NBTTagCompound tagCompound)
    {
        ...
        if (centerCoords == null) // Legacy
        {
            ...
            if (tagCompound.hasKey("centerCoords[0]"))
                centerCoords.set(0, tagCompound.getDouble("centerCoords[0]") - getPos().getX());
            else
                centerCoords.set(0,0.5);

            if (tagCompound.hasKey("centerCoords[1]"))
                centerCoords.set(1, tagCompound.getDouble("centerCoords[1]") - getPos().getY());
            else
                centerCoords.set(1, 0.5);

            if (tagCompound.hasKey("centerCoords[2]"))
                centerCoords.set(2, tagCompound.getDouble("centerCoords[2]") - getPos().getZ());
            else
                centerCoords.set(2, 0.5);
        }

        centerCoordsSize = IvNBTHelper.readDoubleArray("multiBlockSize", tagCompound);
        if (centerCoordsSize == null) // Legacy
        {
            centerCoordsSize = new ArrayList<>();
            if (tagCompound.hasKey("centerCoordsSize[0]"))
                centerCoordsSize.set(0, tagCompound.getDouble("centerCoordsSize[0]"));
            else
                centerCoordsSize.set(0, 0.5);

            if (tagCompound.hasKey("centerCoordsSize[1]"))
                centerCoordsSize.set(1, tagCompound.getDouble("centerCoordsSize[1]"));
            else
                centerCoordsSize.set(1, 0.5);

            if (tagCompound.hasKey("centerCoordsSize[2]"))
                centerCoordsSize.set(2, tagCompound.getDouble("centerCoordsSize[2]"));
            else
                centerCoordsSize.set(2, 0.5);
        }
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound tagCompound)
    {
        ...
        IvNBTHelper.writeDoubleArray("multiBlockCenter", toArr(centerCoords), tagCompound);
        IvNBTHelper.writeDoubleArray("multiBlockSize", toArr(centerCoords), tagCompound);
   ...
   /* public IvRaytraceableAxisAlignedBox getRotatedBox(Object userInfo, double x, double y, double z, double width, double height, double depth)
    ...
    public AxisAlignedBB getRenderBoundingBox()
    {
        ...
        {
            return getBoxAroundCenter(centerCoordsSize.get(0), centerCoordsSize.get(1), centerCoordsSize.get(2));
        }
    }
}
