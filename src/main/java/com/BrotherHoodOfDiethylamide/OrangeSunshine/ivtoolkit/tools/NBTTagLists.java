...
public class NBTTagLists
{
    ...
    {
        int size = 0;
        while (nbt.iterator().hasNext()){
            size++;
            nbt.iterator().next();
        }
        return IntStream.range(0, size).mapToObj(nbt::get).collect(Collectors.toList());
    }
    ...
    public static List<NBTTagCompound> compounds(final NBTTagList nbt)
    {
        ...
        return IntStream.range(0, size).mapToObj(nbt::getCompoundTagAt).collect(Collectors.toList());
    }
    ...
    public static List<int[]> intArrays(final NBTTagList nbt)
    {
        ...
        return IntStream.range(0, size).mapToObj(nbt::getIntArrayAt).collect(Collectors.toList());
    }
    ...
    public static List<NBTTagList> lists(NBTTagList nbt)
    {
        ...
        return (List) IntStream.range(0,size).mapToObj(i -> nbt.get(i).getId() == Constants.NBT.TAG_LIST ? nbt.get(i) : new NBTTagList()).collect(Collectors.toList());
    }
}
