...
import com.BrotherHoodOfDiethylamide.OrangeSunshine.ivtoolkit.world.chunk.gen.MutableBoundingBoxes;

import java.util.Iterator;
import java.util.stream.IntStream;
import java.util.stream.Stream;

import static com.BrotherHoodOfDiethylamide.OrangeSunshine.ivtoolkit.util.IvStreams.flatMapToObj;
...
public class BlockSurfaceArea implements Iterable<BlockSurfacePos>
{
   ...
   /* public static BlockSurfaceArea from(MutableBoundingBoxes boundingBox)
    {
    ...
    public Stream<BlockSurfacePos> stream()
    {
        BlockSurfacePos lower = getLowerCorner();
        BlockSurfacePos higher = getHigherCorner();
        return flatMapToObj(IntStream.range(lower.x, higher.x + 1), x -> IntStream.range(lower.z, higher.z + 1).mapToObj(z -> new BlockSurfacePos(x, z)));
    }
