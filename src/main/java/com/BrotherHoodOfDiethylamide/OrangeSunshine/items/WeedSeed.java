...
public class WeedSeed extends Item implements IHasModel, IPlantable {
	public WeedSeed(String name) {
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        ...
        if(facing == EnumFacing.UP && player.canPlayerEdit(pos, facing, stack) && state.getBlock() == Blocks.FARMLAND && worldIn.isAirBlock(pos.up())) {
            worldIn.setBlockState(pos.up(), Block_init.WeedCrop.getDefaultState());
