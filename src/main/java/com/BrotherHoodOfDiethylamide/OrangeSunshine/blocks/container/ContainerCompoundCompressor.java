...
public class ContainerCompoundCompressor extends Container {
    private final TileCompoundCompressor tile;
    private int[] lastProcess = new int[TileCompoundCompressor.INPUT_SLOTS * 2];

    public ContainerCompoundCompressor(InventoryPlayer playerInventory, TileCompoundCompressor tile) {
        this.tile = tile;
        ...
        addSlotToContainer(new Slot(tile, TileCompoundCompressor.OUTPUT_SLOT, 116, 36));
        for (int row = 0; row < 3; row++) {
    ...
    @Override public void detectAndSendChanges() {
        ...
        for (IContainerListener listener : listeners) {
            for (int i = 0; i < TileCompoundCompressor.INPUT_SLOTS * 2; i++) {
    ...
    @Override public ItemStack transferStackInSlot(EntityPlayer player, int index) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            int tileSlots = TileCompoundCompressor.TOTAL_SLOTS;
            if (index < tileSlots) { if (!mergeItemStack(slotStack, tileSlots, inventorySlots.size(), true)) return ItemStack.EMPTY; }
            else { if (!mergeItemStack(slotStack, 0, TileCompoundCompressor.INPUT_SLOTS, false)) return ItemStack.EMPTY; }
