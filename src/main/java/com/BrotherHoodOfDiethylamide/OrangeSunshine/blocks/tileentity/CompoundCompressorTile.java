    ...
    private ItemStackHandler createHandler() {
        return new ItemStackHandler(10) {
            ...
            public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
                if (slot == 9 ) {
                    List<CompoundCompressorRecipe> recipes =  level.getRecipeManager()
                            .getAllRecipesFor (ModRecipeTypes.COMPOUND_COMPRESSOR_RECIPE);

                    List<ItemStack> outputs = new ArrayList<>();

                    for (CompoundCompressorRecipe recipe: recipes) {
