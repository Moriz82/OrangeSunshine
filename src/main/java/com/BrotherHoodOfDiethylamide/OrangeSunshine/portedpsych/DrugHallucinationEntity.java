...
public class DrugHallucinationEntity extends DrugHallucination
{
    ...
    public DrugHallucinationEntity(EntityPlayer playerEntity)
    {
        ...
        String entityName = hEntities[playerEntity.world.rand.nextInt(hEntities.length)];

        entity = EntityList.createEntityByID(playerEntity.world.rand.nextInt(hEntities.length), playerEntity.world);
    ...
    public void render(float par1, float dAlpha)
    {
        ...
        if (alpha > 0.0f)
        {
           ...
           // Render var10 = RenderManager.getEntityRenderObject(this.entity);
            GL11.glTranslated(var3 - entity.posX, var5 - entity.posY, var7 - entity.posZ);
            GL11.glRotatef(var9, 0, 1, 0);

            GL11.glScaled(scale, scale, scale);
            //entity.render .doRender(this.entity, 0.0F, 0.0F, 0.0F, 0.0F, 1.0f);
