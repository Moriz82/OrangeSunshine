...
import com.BrotherHoodOfDiethylamide.OrangeSunshine.ivtoolkit.legacy.math.IvMatrixHelper;
...
//        return IvMatrixHelper.lookFrom((float) entity.posX, (float) entity.posY, (float) entity.posZ, (entity.rotationYaw + 180.0f) * degToRad, entity.rotationPitch * degToRad, roll * degToRad, projectionMatrix, projectionMatrix);
    ...
    public static Vector3f projectPointCurrentView(Vector3f point, float partialTicks)
    {
        ...
        {
            double f1 = renderViewEntity.getYOffset() - 1.62F;
...
//            double d7 = (double)(entityRenderer.thirdPersonDistanceTemp + (entityRenderer.thirdPersonDistance - entityRenderer.thirdPersonDistanceTemp) * partialTicks);
            ...
            for (int k = 0; k < 8; ++k)
            {
                ...
                f4 *= 0.1F;
                f5 *= 0.1F;
                RayTraceResult movingobjectposition = mc.world.rayTraceBlocks(new Vec3d(d0 + (double) f3, d1 + (double) f4, d2 + (double) f5), new Vec3d(d0 - d3 + (double) f3 + (double) f5, d1 - d5 + (double) f4, d2 - d4 + (double) f5));

                if (movingobjectposition != null)
                {
                    double d6 = movingobjectposition.hitVec.distanceTo(new Vec3d(d0, d1, d2));
    ...
    public static float getCurrentFOV(float partialTicks, boolean isWorld)
    {
        ...
        if (getFOVMethod == null)
            getFOVMethod = ReflectionHelper.findMethod(EntityRenderer.class, "func_78481_a", "getFOVModifier", Float.TYPE, Boolean.TYPE);
