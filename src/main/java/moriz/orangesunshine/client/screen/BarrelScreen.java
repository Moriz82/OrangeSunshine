...
public class BarrelScreen extends FluidProcessingContraptionScreen<BarrelBlockEntity> {
    ...
    public BarrelScreen(FluidContraptionScreenHandler<BarrelBlockEntity> handler, Inventory inventory, Component title) {
    ...
    protected void drawAdditionalInfo(GuiGraphics context, int baseX, int baseY) {
        ...
        if (progress > 0 && progress < 1) {
            context.blit(RenderPipelines.GUI_TEXTURED, background, baseX + 110, baseY + 20, 233, 22, 23, 22, 256, 256);
            int barHeight = (int)(22 * (1 - progress));
            context.blit(RenderPipelines.GUI_TEXTURED, background, baseX + 110, baseY + 20 + barHeight, 233, barHeight, 23, 23 - barHeight, 256, 256);
        }
    }
}
